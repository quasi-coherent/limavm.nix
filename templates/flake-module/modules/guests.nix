{ den, ... }:
{
  ...
  # `limaPackages` publishes vm1 as a flake package so you can start it by hand:
  ...
  # Uses the `limaGuest` den class to configure the lima options tree.
  den.aspects.vm1.limaGuest = {
    lima.runner = {
      cpus = 4;
      memory = "8GiB";
    };
